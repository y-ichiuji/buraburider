...
import type { Coord, PlanResponse } from '../../server/types'
...
export interface UseRoutePlan extends RoutePlanState {
  /** 出発地・目的地からルートを生成する。 */
  generate: (origin: Coord, destination: Coord, detourLevel?: number) => Promise<void>
  /** 生成結果をクリアする。 */
  reset: () => void
}
...
export function useRoutePlan(): UseRoutePlan {
  ...
  const generate = useCallback(async (origin: Coord, destination: Coord, detourLevel?: number) => {
    setState({ plan: null, status: 'loading', error: null })
    try {
      const res = await fetch(PLAN_ENDPOINT, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify(buildPlanRequest(origin, destination, detourLevel))
      })
